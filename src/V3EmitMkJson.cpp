...
class V3EmitMkJsonEmitter final {
    ...
    private:
        ...
        std::stack<char> m_scope;  // stack of ']' and '}' to be used to close currently open scopes
        std::string m_prefix;  // prefix emited before each line in the current scope (indent * scope depth)
        std::string m_indent;  // single indent
        bool m_empty = true;  // indicates if the current scope is empty
    ...
    static string universalAbsolutePath(const string& filename) {
        return V3Os::fileSlashPath(V3Os::filenameRealPath(filename));
    }

    static void emitManifest() {
        const std::string makeDir = universalAbsolutePath(v3Global.opt.makeDir());
        ...
        for (AstNodeFile* nodep = v3Global.rootp()->filesp(); nodep;
            ...
            if (cfilep && cfilep->source()) {
                const std::string filename = universalAbsolutePath(cfilep->name());
                if (cfilep->support()) {
                    if (cfilep->slow()) {
        ...
        const std::string verilatorRoot = universalAbsolutePath(V3Options::getenvVERILATOR_ROOT());
        ...
        for (const auto& dep : V3File::getAllDeps()) deps.emplace_back(universalAbsolutePath(dep));
        for (const auto& cppFile : v3Global.opt.cppFiles())
            cppFiles.emplace_back(universalAbsolutePath(cppFile));
        ...
        if (const V3HierBlockPlan* const planp = v3Global.hierPlanp()) {
            ...
            for (V3HierBlockPlan::HierVector::const_iterator it = hierBlocks.begin();
                ...
                for (const string& i : vFiles) sources.emplace_back(universalAbsolutePath(i));
                ...
                cursor += cursor.begin()
                              ...
                              .put("verilator_args",
                                   universalAbsolutePath(hblockp->commandArgsFilename(true)))
                              .end();
            }
            ...
            cursor += cursor.begin()
                          ...
                          .putList("sources", sources)
                          .put("verilator_args",
                               universalAbsolutePath(planp->topCommandArgsFilename(true)))
                          .end()
                          .end();
        }
    }
